...
import YoadImg from "../../public/images/hero.png"; // החלף בנתיב המתאים לתמונה
...
const HeroSection = () => {
  return (
    <section className="bg-white w-5/6 mx-auto text-black dark:bg-black dark:text-white min-h-[90vh] flex flex-col-reverse md:flex-row-reverse items-center justify-between px-6 md:px-20 py-12 transition-colors duration-300">
      ...
      {/* תמונה */}
      <div className="flex-1 flex justify-center  -mt-12 relative  md:mt-0">
        {/* צורה */}
        <div className="absolute top-2 -right-10 -left-10 md:top-24 md:right-0 h-[100%] md:w-[100%] md:h-[80%] max-h-[90vh] z-10">
          <Image
            src={Shape}
            alt="צורה לא סימטרית"
            layout="fill"
        ...
        <Image
          ...
          width={400}
          height={400}
          className="relative z-20 ml-5 w-[100%] md:w-[600px] max-h-[50vh] md:max-h-[90vh] object-contain"
        />
        {/* דיב שחור עם בורדר לבן */}
        <div className="absolute bottom-24 -right-0 font-rubik md:right-16 md:bottom-28 bg-black dark:bg-white text-white dark:text-black border-2 border-white dark:border-black px-3 py-1 text-sm font-semibold rounded-full shadow-lg z-30">
          17 שנות ניסיון{" "}
        </div>

        {/* דיב ירוק עם בורדר שחור */}
        <div className="absolute bottom-12 md:bottom-20 font-rubik left-0 md:left-16 bg-[#32FF72] dark:bg-[#28D65A] text-black dark:text-white border-2 border-black dark:border-white px-3 py-1 text-sm font-semibold rounded-full shadow-lg z-30">
          3 ימים וזה אצלכם
        </div>
      </div>
    </section>
